...
export function InviteContainer(props: {
    requestId?: string; groupId?: string; type: "INVITE" | "JOIN";
}) {
    ...
    function fetchGroupJoinDetails() {
        _props
            ...
            .then(result => {
                _data({
                    group: result.data, user: result.data.User, request: result.data.Request,
                });
                _loading(false);
            });
    }
    ...
    async function handleGroupJoinRequest() {
        ...
        if (data?.request[0]?.id) {
            let response = await _props
                ._db(service.group)
                .query(serviceRoute.updateInvite, {status: "REJECTED"}, reqType.delete, data.request[0].id)
            fetchGroupJoinDetails();
            _loading(false);
            _message(response.message);
        } else {
            try {
                let response = await _props._db(service.group).query(serviceRoute.groupInvite, {}, reqType.post, data.group.id)
                fetchGroupJoinDetails();
                _loading(false);
                _message(response.message);
            } catch (e) {
    ...
    function handleGroupJoin() {
        ...
        if (d) {
            ...
            _props
                ._db(service.group)
                .query(serviceRoute.updateInvite, {status: "REJECTED"}, reqType.delete, d.id)
                .then(() => {
    ...
    function handleInviteReject() {
        ...
        _props
            ._db(service.group)
            .query(serviceRoute.groupInvite, {status: "REJECTED"}, reqType.put, props.requestId)
            .then(result => {
                _loading(false);
                _setRefetch(!refetch);
                _setGroupType(null);
        ...
        {loading ? (<Spinner/>) : (<>
            <div className={"inviteWrapper"}>
                <div className={"invContainer"}>
                    ...
                    {props.type === "INVITE" ? (<div style={{display: "flex"}}>
                        ...
                        <div style={{display: "flex"}}>
                            <div className={"btn-Wrapper"}>
                                ...
                                >
                                    {data.request.length === 0 ? "Request" : "Cancel Request"}
                                </div>
                            </div>
                        </div>
                    </>)}
