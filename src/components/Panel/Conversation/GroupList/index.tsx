...
export function GroupList({
  ...
  const {userId, groupId, sidePanel, updateSidePanelState} =
            ...
            {item?.Message?.length > 0 && groupId !== item.id && (
              <div style={{display: "flex", fontSize: "13px"}}>
                <div style={{fontSize: "13px", color: "rgb(14 151 131)"}}>
                  {item?.Message[0]?.User.id === userId
                    ? "You"
                    : item?.Message[0]?.User?.name.split(" ")[0]}
                </div>
                &nbsp;
                <div
                  style={{
                ...
                >
                  {item?.Message[0]?.MessageContent?.TYPE === "TEXT"
                    ? item?.Message[0]?.MessageContent?.content
                    : "Sent an image"}
                </div>
              </div>
            )}
            ...
            <div style={{width: "22%"}}>
              <div
                ...
                style={{
                  ...
                  color:
                    !hasSeen &&
                    item.Message[0]?.ReadReceipt[0].status === "Sent"
                      ? "#0e9783"
                      : "whitesmoke",
                  display: "flex",
              ...
              >
                {item?.Message?.length > 0 &&
                  formatDateToDDMMYYYY(
                    item.Message[item?.Message?.length - 1].created_at
                  )}
                {item.Message[0]?.ReadReceipt[0].status == "Sent" &&
                  !hasSeen && <PiHandHeartFill size={22} />}
              </div>
            </div>
          )}
        </div>
